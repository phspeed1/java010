...
public class Inf070MinSpaningTree{
    ...
    public static void main(String[] args){
        ...
        String rtn = new Inf070MinSpaningTree().solution(V, E, arr);
    ...
    private String solution(int V, int E, int[][] arr){
        ...
        while(!pQueue.isEmpty()){
            ...
            int now = nowNode.edge;
            int nowCost = nowNode.cost;
            if(nowCost > dis[now]) continue;
            for(Node link : list.get(now)){
                if(nowCost + link.cost < dis[link.edge]){
                    dis[link.edge] = nowCost + link.cost;
                    pQueue.offer(new Node(link.edge, nowCost + link.cost));
                }
            }
        }
